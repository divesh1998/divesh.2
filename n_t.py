...
def calculate_rsi(df, period=14): delta = df['Close'].diff() gain = (delta.where(delta > 0, 0)).rolling(window=period).mean() loss = (-delta.where(delta < 0, 0)).rolling(window=period).mean() rs = gain / loss df['RSI'] = 100 - (100 / (1 + rs)) return df
...
def generate_signals(df): df['EMA10'] = df['Close'].ewm(span=10).mean() df['EMA20'] = df['Close'].ewm(span=20).mean() df = calculate_rsi(df) df['Signal'] = 0 df.loc[(df['EMA10'] > df['EMA20']) & (df['RSI'] > 50), 'Signal'] = 1 df.loc[(df['EMA10'] < df['EMA20']) & (df['RSI'] < 50), 'Signal'] = -1 return df
...
def backtest_strategy_accuracy(df, use_elliott=False, use_price_action=False): df = df.copy() df['EMA10'] = df['Close'].ewm(span=10).mean() df['EMA20'] = df['Close'].ewm(span=20).mean() df = calculate_rsi(df) df['Signal'] = 0 df.loc[(df['EMA10'] > df['EMA20']) & (df['RSI'] > 50), 'Signal'] = 1 df.loc[(df['EMA10'] < df['EMA20']) & (df['RSI'] < 50), 'Signal'] = -1 if use_elliott: breakout, _ = detect_elliott_wave_breakout(df) if not breakout: df['Signal'] = 0 if use_price_action: patterns = detect_price_action(df) if not patterns: df['Signal'] = 0 df['Return'] = df['Close'].pct_change().shift(-1) df['StrategyReturn'] = df['Signal'].shift(1) * df['Return'] total_signals = df[df['Signal'] != 0] correct = df[df['StrategyReturn'] > 0] accuracy = round(len(correct) / len(total_signals) * 100, 2) if len(total_signals) else 0 return accuracy

--- Upload Chart Feature ---
...
st.metric("📘 EMA+RSI Accuracy", f"{acc_ema}%")
